...
batch_x = np.ones([6, 30], dtype=float)
batch_y = np.ones([6], dtype=float)
...
for i in range(6):
	...
	Target = Source_Data[T + (i+6)]					# randomly pick T + [6-12] as target value
	for e in range(30):
...
with tf.Session() as sess:
	...
	print("Actual:")
	for i in range(6):
		print(scaler.inverse_transform(batch_y[i]))
